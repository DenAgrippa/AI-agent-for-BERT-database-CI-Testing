...
class TestAgent(unittest.TestCase):
    ...
    @patch("agent.process_prompt")
    @patch("agent.init_db")
    def test_main_with_prompt_regular(self, mock_init_db, mock_process_prompt, mock_llama):
        mock_process_prompt.return_value = 0
        mock_llama_instance = mock_llama.return_value
        mock_llama_instance.generate_llama_response.return_value = "regular response"
